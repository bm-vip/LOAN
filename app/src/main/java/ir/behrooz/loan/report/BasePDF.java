...
public abstract class BasePDF extends PdfPageEventHelper {
    ...
    protected PdfPCell createPdfPCell(int i, String text, Font font, int border) {
        return createPdfPCell(i, text, font, border, Element.ALIGN_CENTER);
    }
...
//        float center = (pageSize.LEFT + pageSize.RIGHT) / 2;
    ...
    protected void pageHeader(Document document, String title) throws DocumentException {
        ...
        dateTbl.addCell(createPdfPCell(0, DateUtil.toPersianString(new Date(), false), font_LT_12, 0, Element.ALIGN_RIGHT));
        dateTbl.addCell(createPdfPCell(0, context.getString(R.string.date), font_MD_14, 0, Element.ALIGN_RIGHT));

        PdfPCell pdfPCell = new PdfPCell(dateTbl);
        pdfPCell.setBorder(0);
        table.addCell(pdfPCell);
        table.addCell(createPdfPCell(0, title, font_BL_16, 0));
        table.addCell(createPdfPCell(0, cashtEntity.getName(), font_MD_14, 0, Element.ALIGN_LEFT));
