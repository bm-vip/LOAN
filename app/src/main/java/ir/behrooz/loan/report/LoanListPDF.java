...
public class LoanListPDF extends BasePDF {
    ...
    protected void createTable(Paragraph paragraph, Object list) {
        ...
        for (int i = 0; i < ((List) list).size(); i++) {
            ...
            tbl6.addCell(createPdfPCell(i, context.getString(R.string.installment), font_MD_14, 0));
            tbl6.addCell(createPdfPCell(i, LanguageUtils.getPersianNumbers(entity.getInstallment() + ""), font_LT_12, 0));
            table.addCell(tbl6);

            PdfPTable tbl7 = new PdfPTable(1);
            tbl7.addCell(createPdfPCell(i, context.getString(R.string.dayOfMonth), font_MD_14, 0));
            tbl7.addCell(createPdfPCell(i, LanguageUtils.getPersianNumbers(entity.getDayInMonth() + ""), font_LT_12, 0));
            table.addCell(tbl7);

            PdfPTable tbl8 = new PdfPTable(1);
            tbl8.addCell(createPdfPCell(i, context.getString(R.string.loanAmount), font_MD_14, 0));
            tbl8.addCell(createPdfPCell(i, moneySeparator(context, entity.getValue()), font_LT_12, 0));
            loanSum += entity.getValue();
            table.addCell(tbl8);

            PdfPTable tbl9 = new PdfPTable(1);
            tbl9.addCell(createPdfPCell(i, context.getString(R.string.loanTitle), font_MD_14, 0));
            tbl9.addCell(createPdfPCell(i, entity.getName(), font_LT_12, 0));
            table.addCell(tbl9);

            PdfPTable tbl10 = new PdfPTable(1);
            tbl10.addCell(createPdfPCell(i, context.getString(R.string.name), font_MD_14, 0));
            PersonEntity personEntity = DBUtil.getReadableInstance(context).getPersonEntityDao().load(entity.getPersonId());
            tbl10.addCell(createPdfPCell(i, String.format("%s %s", personEntity.getName(), personEntity.getFamily()), font_LT_12, 0));
            table.addCell(tbl10);

            PdfPTable tbl1 = new PdfPTable(1);
            tbl1.addCell(createPdfPCell(i, context.getString(R.string.receiveDate), font_MD_14, 0));
            tbl1.addCell(createPdfPCell(i, DateUtil.toPersianString(entity.getDate(), false), font_LT_12, 0));
            table.addCell(tbl1);
            ...
            tbl2.addCell(createPdfPCell(i, context.getString(R.string.delayed), font_MD_14, 0));
            tbl2.addCell(createPdfPCell(i, LanguageUtils.getPersianNumbers(delayedCount + ""), font_LT_12, 0));
            table.addCell(tbl2);

            PdfPTable tbl3 = new PdfPTable(1);
            tbl3.addCell(createPdfPCell(i, context.getString(R.string.remainLoan), font_MD_14, 0));
            ...
            remainSum += remain;
            tbl3.addCell(createPdfPCell(i, moneySeparator(context, remain), font_LT_12, 0));
            table.addCell(tbl3);

            PdfPTable tbl4 = new PdfPTable(1);
            tbl4.addCell(createPdfPCell(i, context.getString(R.string.settled), font_MD_14, 0));
            tbl4.addCell(createPdfPCell(i, entity.getSettled() ? context.getString(R.string.done) : context.getString(R.string.not), font_LT_12, 0));
            table.addCell(tbl4);

            PdfPTable tbl5 = new PdfPTable(1);
            tbl5.addCell(createPdfPCell(i, context.getString(R.string.installmentAmount), font_MD_14, 0));
            tbl5.addCell(createPdfPCell(i, moneySeparator(context, entity.getInstallmentAmount()), font_LT_12, 0));
            table.addCell(tbl5);
